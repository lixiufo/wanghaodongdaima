...
<body>
		...
		<div class="container">
			...
			<div class="collapse navbar-collapse"
				...
				<ul class="nav navbar-nav">
					<li><a href="<%=path %>/zhumohu" method="post">商品管理 <span class="sr-only">(current)</span></a></li>
	...
	<div class="container">
		<form class="form-inline"
			action="${pageContext.request.contextPath}/userFuzzy"
			method="post">
			<div class="col-md-4">
	...
	<div class="container" style="margin-top:20px;">
			...
			<table style="text-align: center;margin-right:10px;width:99%"
				...
				<td>${user.username}</td>
					<td>${user.password}</td>
		                  ...
		                  <td><a href="<%=path%>/delStu1?userid=${user.id}">删除</a></td>
 	...
 	%>
 		...
 		<td><a href="<%=path%>/userFuzzy?pageNo=1">首页</a></td>
 		<td><a href="<%=path%>/userFuzzy?pageNo=${pageNo-1}">上一页</a></td>
 		<%
 		}
 		if(pageNo<pageCount){
 		%>
 		<td><a href="<%=path%>/userFuzzy?pageNo=${pageNo+1}">下一页</a></td>
 		<td><a href="<%=path%>/userFuzzy?pageNo=${pageCount}">尾页</a></td>
 		<%
 		}
 	%>
 	</tr>
